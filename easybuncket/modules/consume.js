...
Consume.del = function del(id,callback){
	mongodb.open(function(err,db){
        ...
        db.collection('consume', function(err, collection){
			...
			collection.remove(query,{safe:true},function(err,consumeresult){
				mongodb.close();
		        if (err){
		            return callback(err); 
		        }
    ...
    mongodb.open(function(err, db){
        ...
        db.collection('consume', function(err, collection){
            ...
            collection.group(
        		...
        		{count:0,amount:0},
            	reduce,
            	function(err, consumeresult){
                	mongodb.close(); 
	                if (err){
	                    callback(err, null); 
	                }
	                else{
	                	console.log(consumeresult[0]);
	                	var amount = 0,count=0;
                        consumeresult.forEach(function (item,index){
	                		amount += item.amount;
	                		count += item.count;
	                	});
	                	consumeresult.push({consumeSubject:'【合计】',count:count,amount:amount});
                        //var ConsumeSum=amount;
	                	...
	                	*/
	                	...
	                	collection.find().toArray(function(err,docs){
	                		docs.forEach(function(item,idx){
                                ...
                                consumeresults.push({consumeSubject:item._id,amount:item.value.amount,count:item.value.count});
	                		});
	                		mongodb.close();
		                	callback(null, consumeresults);
	                	});
	                }
	        });
        }); 
